        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="space-y-3">
            <div class="flex items-center space-x-2">
              ...
              <label class="text-white font-medium">{{ t("partner.type") }}</label>
            </div>
            <div class="flex space-x-4">
              <label v-for="type in userTypes" :key="String(type.value)" class="flex items-center space-x-2 cursor-pointer">
                <input
                  type="radio"
                  :value="type.value"
                  v-model="form.isshop"
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="space-y-2">
              <div class="flex items-center space-x-2">
                ...
                <label class="text-white font-medium">{{ t("register.username") }}</label>
              </div>
              <Input
                ...
                class="bg-white/10 border-white/20 text-white placeholder-gray-400 focus:border-blue-500"
              />
            ...
            <div class="space-y-2">
              <div class="flex items-center space-x-2">
                <UserCheck class="w-5 h-5 text-purple-400" />
                <label class="text-white font-medium">{{ t("register.name") }}</label>
              </div>
              <Input
                ...
                class="bg-white/10 border-white/20 text-white placeholder-gray-400 focus:border-blue-500"
              />
              <p v-if="errors.nickname" class="text-red-400 text-sm">{{ errors.nickname }}</p>
            </div>
            ...
            <div class="space-y-2">
              <div class="flex items-center space-x-2">
                ...
                <label class="text-white font-medium">{{ t("register.password") }}</label>
              </div>
              <div class="relative">
                <Input
                  ...
                  class="bg-white/10 border-white/20 text-white placeholder-gray-400 focus:border-blue-500 pr-12"
                />
                <button
                  type="button"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div class="space-y-2">
                <div class="flex items-center space-x-2">
                  ...
                  <label class="text-white font-medium">{{ t("register.mobileNumber") }}</label>
                </div>
                <Input
                  v-model="form.telno"
                  type="text"
                  ...
                  class="bg-white/10 border-white/20 text-white placeholder-gray-400 focus:border-blue-500"
                />
                <p v-if="errors.telno" class="text-red-400 text-sm">{{ errors.telno }}</p>
              </div>
              ...
              <div class="space-y-2">
                <div class="flex items-center space-x-2">
                  ...
                  <label class="text-white font-medium">{{ t("register.bankName") }}</label>
                </div>
                <select
                  v-model="form.bank_name"
                  :class="{ 'border-red-500': errors.bank_name }"
                  class="w-full bg-white/10 border border-white/20 rounded-lg text-white px-3 py-2 focus:border-blue-500 focus:outline-none"
                >
                  <option v-for="bank in banks" :key="bank" :value="bank" class="bg-gray-800 text-white">
                    {{ bank }}
                  </option>
                </select>
              ...
              <div class="space-y-2">
                <div class="flex items-center space-x-2">
                  <UserCheck class="w-5 h-5 text-purple-400" />
                  <label class="text-white font-medium">{{ t("register.bankAccountName") }}</label>
                </div>
                <Input
              ...
              <div class="space-y-2 md:col-span-2 lg:col-span-1">
                <div class="flex items-center space-x-2">
                  <CreditCard class="w-5 h-5 text-orange-400" />
                  <label class="text-white font-medium">{{ t("register.bankAccount") }}</label>
                </div>
                <Input
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div class="space-y-2">
                <div class="flex items-center space-x-2">
                  ...
                  <label class="text-white font-medium">{{ t("commission.typeRolling") }}</label>
                </div>
                <select
                  v-model="form.type_rolling"
                  disabled
                  class="w-full bg-white/5 border border-white/10 rounded-lg text-gray-400 px-3 py-2 cursor-not-allowed"
                >
                  <option v-for="type in rollingTypes" :key="type.value" :value="type.value" class="bg-gray-800">
                    {{ type.label }}
                  </option>
                </select>
              </div>
              ...
              <div class="space-y-2">
                <div class="flex items-center space-x-2">
                  <Dices class="w-5 h-5 text-red-400" />
                  <label class="text-white font-medium">{{ t("commission.casinoRollingPercent") }}</label>
                </div>
                <Input
                  ...
                  class="bg-white/10 border-white/20 text-white placeholder-gray-400 focus:border-blue-500"
                />
                <p v-if="errors.pct_roll_casino" class="text-red-400 text-sm">{{ errors.pct_roll_casino }}</p>
              </div>
              ...
              <div class="space-y-2">
                <div class="flex items-center space-x-2">
                  ...
                  <label class="text-white font-medium">{{ t("commission.slotRollingPercent") }}</label>
                </div>
                <Input
                  ...
                  class="bg-white/10 border-white/20 text-white placeholder-gray-400 focus:border-blue-500"
                />
                <p v-if="errors.pct_roll_slot" class="text-red-400 text-sm">{{ errors.pct_roll_slot }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-end space-x-3 pt-4 border-t border-white/10">
            ...
            <button
              ...
              class="px-6 py-2 bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-400 hover:to-blue-500 text-white rounded-lg font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
const banks = [
  "KB국민은행", "신한은행", "우리은행", "KEB하나은행", "카카오뱅크", 
  "카카오증권", "KDB산업은행", "IBK기업은행", "NH농협은행", "수협은행", "대구은행", 
  "BNK부산은행", "BNK경남은행", "광주은행", "전북은행", "제주은행", "농·축협", "농협", "축협", 
  "새마을금고", "우체국", "신용협동조합", "산림조합", "HSBC은행", "한국씨티은행", 
  "한국스탠다드차타드은행", "미래애셋", "SC제일은행"
]
...
const validationSchema = computed(() => {
  const baseSchema = {
    ...
    pct_roll_casino: z.number()
      .min(0, t('Partner.Validation.Required'))
      .max(100, 'Maximum 100%'),
    pct_roll_slot: z.number()
      .min(0, t('Partner.Validation.Required'))
      .max(100, 'Maximum 100%')
  }
...
const getMasterSettlementTypes = async () => {
  if (authStore.user.id) {
    try {
      const response = await ApiService.get(`/partner/member/settlement-type/${authStore.user.id}`)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      password: form.password,
      name: form.nickname,
      mobile: form.isshop ? Array.from({ length: 11 }, () => Math.floor(Math.random() * 10)).join('') : form.telno,
      bank_name: form.isshop ? "카카오방크" : form.bank_name,
      bank_account_name: form.isshop ? Math.random().toString(36).substring(2, 9) : form.bank_account_name,
      bank_account: form.isshop ? Array.from({ length: 11 }, () => Math.floor(Math.random() * 10)).join('') : form.bank_account,
      pct_roll_casino: Number(form.pct_roll_casino),
      pct_roll_slot: Number(form.pct_roll_slot),
      pct_lose_casino: 0,
      pct_lose_slot: 0,
