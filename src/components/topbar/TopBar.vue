  ...
  <div class="bg-[#07102e] py-2 text-sm border-b border-[#0c2b5f]">
    <div class="max-w-[1660px] mx-auto px-4">
      <div class="flex items-center justify-between md:justify-end gap-2">
        ...
        <div v-if="!authStore.isAuthenticated" class="flex items-center space-x-1 md:space-x-2">
          <!-- Login Button -->
          <button @click="navigateToLogin" class="cursor-pointer flex items-center space-x-1 px-1.5 md:px-2 py-1.5 rounded text-gray-300 hover:text-white transition-colors">
          ...
          <!-- Sign Up Button -->
          <button @click="navigateToSignup" class="cursor-pointer flex items-center space-x-1 px-1.5 md:px-2 py-1.5 rounded text-gray-300 hover:text-white transition-colors">
            <div class="relative">
              <svg class="w-3 h-3 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
              </svg>
              <svg class="w-2 h-2 md:w-3 md:h-3 absolute -bottom-1 -right-1 text-blue-400" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="authStore.isAuthenticated" class="flex items-center space-x-2 md:space-x-4">
          <!-- M Counter -->
          <div class="flex items-center space-x-1 md:space-x-2">
            <div class="w-5 h-5 md:w-6 md:h-6 bg-blue-400 rounded-full flex items-center justify-center border border-blue-500">
          ...
          <div class="flex items-center space-x-1 md:space-x-2">
            ...
            <span class="text-white font-medium text-xs md:text-sm">{{ n(Number(user.wallet_point)) }}</span>
          </div>
        </div>
