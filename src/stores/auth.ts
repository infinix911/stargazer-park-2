...
export const useAuthStore = defineStore("auth", {
  ...
  actions: {
    ...
    async connectSocket() {
      try {
        ...
        const socketURL = import.meta.env.DEV
          ? String(import.meta.env.VITE_APP_WS)
          : `https://adsc.${rootDomain}`;
        ...
        SocketService.socket.on(
          "info",
          (member: IMember) => (this.user = member)
        );
        // Wallet
    ...
    async updateWalletWithSlot() {
      ...
      this.user.wallet_slot = Number(slotBalance);
      setTimeout(() => {
        this.isAnimateBalance = false;
      }, 200);
    },
  },
});